...
Description: Just for educational purposes, not to be used in any productive manner.
...
STORED AS TEXTFILE
LOCATION '/user/hadoop/imdb/title_ratings'
...
FIELDS TERMINATED BY '\t'
STORED AS TEXTFILE
LOCATION '/user/hadoop/imdb/title_basics'
TBLPROPERTIES ('skip.header.line.count'='1');
"""
...
LOCATION '/user/hadoop/imdb/title_ratings/{{ macros.ds_format(ds, "%Y-%m-%d", "%Y")}}/
{{ macros.ds_format(ds, "%Y-%m-%d", "%m")}}/{{ macros.ds_format(ds, "%Y-%m-%d", "%d")}}/';
"""
...
LOCATION '/user/hadoop/imdb/title_basics/{{ macros.ds_format(ds, "%Y-%m-%d", "%Y")}}/
{{ macros.ds_format(ds, "%Y-%m-%d", "%m")}}/{{ macros.ds_format(ds, "%Y-%m-%d", "%d")}}/';
"""
...
FIELDS TERMINATED BY ','
STORED AS TEXTFILE
LOCATION '/user/hadoop/imdb_final/top_tvseries';
"""
...
STORED AS ORCFILE
LOCATION '/user/hadoop/imdb_final/top_movies';
"""
...
WHERE
    m.partition_year = {{ macros.ds_format(ds, "%Y-%m-%d", "%Y")}}
    AND m.partition_month = {{ macros.ds_format(ds, "%Y-%m-%d", "%m")}}
    AND m.partition_day = {{ macros.ds_format(ds, "%Y-%m-%d", "%d")}}
    AND r.partition_year = {{ macros.ds_format(ds, "%Y-%m-%d", "%Y")}}
    AND r.partition_month = {{ macros.ds_format(ds, "%Y-%m-%d", "%m")}}
    AND r.partition_day = {{ macros.ds_format(ds, "%Y-%m-%d", "%d")}}
    AND r.num_votes > 200000
    AND r.average_rating > 8.6
    AND m.title_type = 'movie'
    AND m.start_year > 2000;
"""
...
pyspark_top_tvseries = SparkSubmitOperator(
    ...
    application_args=[
        ...
        "/user/hadoop/imdb",
        "--hdfs_target_dir",
        "/user/hadoop/imdb_final/top_tvseries",
        "--hdfs_target_format",
        "csv",
    ],
    dag=dag,
